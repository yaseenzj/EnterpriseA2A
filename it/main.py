...
app = FastAPI(title="IT Agent Microservice", version="1.0.0")
...
def register_agent():
    ...
    try:
        with psycopg.connect(DB_URI) as conn:
            ...
            conn.commit()
        logger.info("Successfully registered IT Agent in PostgreSQL agent_registry.")
    ...
    try:
        with psycopg.connect(DB_URI) as conn:
            with conn.cursor() as cur:
                ...
                """, (AGENT_CARD["agent_name"],))
            conn.commit()
        logger.info("Successfully marked IT Agent as OFFLINE in PostgreSQL.")
    except Exception as e:
